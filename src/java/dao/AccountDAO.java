...
public int updateInfo2(Account t) {
        int ketQua = 0;
        try {
            ...
            String sql = "UPDATE account " + " SET " + " email=?" + ", name=?"
                    + ", gender=?" + ", dateOfBirth=?" + ", phoneNumber=?" + " WHERE userID=?";
    ...
    public List<Account> getAll() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Account c = new Account(rs.getInt("userID"), rs.getString("userName"), rs.getString("email"), rs.getString("name"), rs.getString("gender"), rs.getString("dateOfBirth"), rs.getString("phoneNumber"), rs.getString("password"), rs.getInt("roleID"), rs.getString("avata"));
                list.add(c);
            }
        } catch (SQLException e) {
...
public int insertStaff(Account c) {
        int ketQua = 0;

        // Kiểm tra các trường của đối tượng Account để đảm bảo không có trường nào bị null
        if (c.getUserName() == null || c.getEmail() == null || c.getName() == null
                || c.getGender() == null || c.getPhoneNumber() == null
                || c.getDateOfBirth() == null || c.getPassword() == null
                || c.getAvata() == null ||c.getStatusOtp()==null) {
            System.out.println("One or more fields are null");
            return ketQua;
        }

        String sql = "INSERT INTO Account(userName, email, name, gender, phoneNumber, dateOfBirth, password, roleID, avata,statusOtp) VALUES (?, ?, ?, ?, ?, ?, ?, 2, ?,?)";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
            ...
            st.setString(8, c.getAvata());
            st.setBoolean(9, c.getStatusOtp());
            ketQua = st.executeUpdate();
        } catch (SQLException e) {
    ...
    public boolean deleteStaffAccount(int userID) throws SQLException {
    ...
    public int updateStaff(Account a, int userID) {
        int ketQua = 0;
        try {
            // Bước 1: tạo kết nối đến CSDL
            ...
            st.setString(3, a.getPhoneNumber());
            st.setString(4, a.getEmail());
            st.setString(5, a.getDateOfBirth());
            st.setInt(6, userID);
    ...
    public List<Account> getAllStaffByRoleID(int roleID) {
        List<Account> list = new ArrayList<>();
    ...
    public Account selectById1(int ID) {
        Account ketQua = null;
        String xUserName, xPassWord, xEmail, xName, xGender, xPhoneNumber, xAvata;
        int xRole, xId;
        String xDateOfBirth;
        try {
    ...
    public Account selectByUsernameAndPassWord(Account t) {
        ...
        String xDateOfBirth;
        try {
// Bước 1: tạo kết nối đến CSDL/-strong/-heart:>:o:-((:-h// Bước 2: tạo ra đối tượng statement
    ...
    public int insert(Account c) {
        int ketQua = 0;
        String sql = "insert into Account(userName,email,name,gender,phoneNumber,dateOfBirth,password,roleID,avata)" + " values(?,?,?,?,?,?,?,?,?)";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
        ...
        public int insert1(Account c) {
        int ketQua = 0;
        String sql = "insert into Account(userName,email,name,password,roleID,avata,statusOtp)" + " values(?,?,?,?,?,?,?)";
        try {
            PreparedStatement st = connection.prepareStatement(sql);
...
public int updateInfo(Account t) {
        ...
        try {
            ...
            // Bước 2: tạo ra đối tượng statement
            String sql = "UPDATE account " + " SET " + " email=?" + ", name=?"
                    + ", gender=?" + ", dateOfBirth=?" + ", phoneNumber=?"+",avata=?" + " WHERE userID=?";
            PreparedStatement st = connection.prepareStatement(sql);
...
public String returnEmail(int userID) throws SQLException {
    String email = null;
    String sql = "SELECT email FROM Account WHERE userID = ?";
    PreparedStatement st = connection.prepareStatement(sql);
    st.setInt(1, userID);
    ResultSet rs = st.executeQuery();
    if (rs.next()) { // Kiểm tra xem có kết quả từ câu truy vấn không
...
//        System.out.println(c.changePassword(t));
        ...
        Account a = new Account(4, "truongnguyen", "truongnguyenàas", "a", "b", "c", "d", "a", 3, "10");
        d = c.selectById1(a.getID());
...
//            }
//        }
//       Account ac =  new Account("", "truong", "", "", "", "", "", "a", 3, "");/-strong/-heart:>:o:-((:-h//       Account kq = c.selectByUsernameAndPassWord(ac);
//       System.out.println(kq.getDateOfBirth());
        System.out.println(c.returnEmail(5));
    }
}
